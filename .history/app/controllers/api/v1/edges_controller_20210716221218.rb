class Api::V1::EdgesController < Api::V1::BaseController
    ...
    def referredStation
        ...
        Station.find_by(provided_id: station_id)
    end
    ...
    def create
        begin
            if Station.find_by(provided_id: edge_params[:provided_src_id]).nil? ||
                ...
                raise EdgeError.new("Either provided source id or provided destination id is invalid")
            end
            ...
            edge = Edge.new()
            edge.provided_src_id = edge_params[:provided_src_id]
            edge.provided_dest_id = edge_params[:provided_dest_id]
            src_station = Station.find_by(provided_id: edge_params[:provided_src_id])
            dest_station = Station.find_by(provided_id: edge_params[:provided_dest_id])
