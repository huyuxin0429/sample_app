...
edges.each{ |edge| 
    # byebug
    src_station_id = edge[0]
    dest_stations_ids = edge[1]
    src_station = Station.find(src_station_id)
    dest_stations_ids.each{ |dest_station_id| 
        dest_station = Station.find(dest_station_id)
        new_edge = Edge.new(cost: src_station.address.distance_to(dest_station.address))
        new_edge.src_id = src_station.id
        new_edge.dest_id = dest_station.id
        new_edge.stations << src_station
        new_edge.stations << dest_station
        new_edge.save!
