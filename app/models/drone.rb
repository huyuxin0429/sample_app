...
class Drone < ApplicationRecord
  ...
  def route2csv
    address_routes = self.address_id_route.map{ |id|
      [
        Address.find(id).latitude,
        Address.find(id).longitude
      ]
  ...
  def deliver(order)
    ...
    address_id_route = GraphModules.path_address_id(nearestStation2me.id, nearestStation2merchant.id)
    address_id_route << merchant_addresss.id
    address_id_route = address_id_route.concat GraphModules.path_address_id(nearestStation2merchant.id, nearestStation2cust.id)
  ...
  private
    ...
    def approach(target_id, time_delta)
      ...
      if (curr_distance < new_distance)
        ...
        # else
        #   if !self.address_id_route[0].nil?
        #     destination_address_id = self.address_id_route[0]
        #   else
        #     if free_moving?
        #       next_status
        #     end
